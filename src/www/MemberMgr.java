...
public class MemberMgr {
	...
	public boolean loginMember(String id, String pass) {
		...
		try {
			...
			sql = "select id from tblMember where id=? and pass=?"; /*
																	 ...
																	 */
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, id); /* id와 아랫줄 pass는 값을 받아와서 실행하는 변수이다 */
	...
	public boolean deleteId(String id, String pw){
		...
		try {
			con = pool.getConnection();
			sql = "select pass from tblMember where id = ?";			
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			if(rs.next()) {
				...
				if(dbpw.equals(pw)) {
					delsql = "delete from tblMember where id = ?";
	...
	// 관리자-회원삭제하기
	public boolean adminDeleteId(String id, String pw){
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		ResultSet rs = null;
